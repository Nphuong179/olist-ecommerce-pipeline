    ...
    operational_failures_df = run_query("""
        WITH operational_orders AS (
            SELECT
                fo.order_id,
                fo.order_status,
                BOOL_AND(foi.met_shipping_deadline) AS all_met_shipping_deadline
    ...
    low_review_df = run_query("""
        WITH quality_reviews AS (
            SELECT
                ...
                mcvg.customer_id,
                forv.review_score
            FROM dev.main_facts.fact_order_reviews forv
            JOIN dev.main_facts.fact_orders fo 
                ON forv.order_id = fo.order_id 
            JOIN dev.main_dims.dim_customers dc 
                ON fo.customer_key = dc.customer_key 
            JOIN dev.main_marts.mart_customer_value_growth mcvg 
                ON dc.customer_id = mcvg.customer_id 
...
with tab_economic:
    ...
    promo_dependent_df = run_query("""
        WITH avg_nmv_percentiles AS (    
            SELECT
                PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY avg_nmv_per_order) AS p50,
                PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY avg_nmv_per_order) AS p75,
                PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY avg_nmv_per_order) AS p95 
            FROM dev.main_marts.mart_customer_value_growth
            WHERE lifecycle_stage LIKE '%_at_risk'
